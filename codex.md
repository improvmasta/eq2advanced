...
New app served at https://eq2advanced.com.
...
- Zoraxy routes https://eq2advanced.com to 10.1.1.15:8450
...
served by the API). Dev binds 0.0.0.0:8450 -> http://10.1.1.15:8450. The container on
10.1.1.5 is Lindsay's to deploy — never auto-ship, never deploy.
**As of 2026-08-03 https://eq2advanced.com points at the DEV box**. Flip it
back with `provision-app.sh route eq2advanced 8450 --deploy-server media`. The
Cloudflare proxy was on for part of 2026-08-03 and Lindsay turned it OFF: the edge caps
a request body at 100 MB and answers a bigger upload with its own 413, which the app
never sees. `siteconfig.edge_max_bytes()` reports that ceiling per request (`CF-Ray`),
so the dropzone can warn in advance; with the proxy off it is 0. The app reads the real
visitor address out of
`CF-Connecting-IP`/`X-Forwarded-For` via `backend/siteconfig.py` — see ARCHITECTURE.md
"The app is behind two proxies", and do NOT go back to `request.client.host`.
Read `ARCHITECTURE.md` before touching parser/segmentation
(subject model: bare logger-name = their PET; possessive hazards are test-covered).
Tests: `.venv/bin/python -m pytest backend/tests/ -q` (198; golden fixture = /home/lindsay/bobby.txt).
...
The brute-force counter was the urgent one — behind Zoraxy every request carried
the same client address, so `ratelimit`'s per-address bucket was ONE counter for
the whole internet and five wrong passwords by anyone locked everybody out of
login for fifteen minutes. `client_ip` reads `CF-Connecting-IP`/`X-Forwarded-For`
only from a trusted peer (`TRUSTED_PROXIES`, default 10.1.1.4 + loopback);
`/auth/password` and `/auth/security-question` are counted too (both are password
oracles); `is_secure` restores the `Secure` flag edge TLS was dropping; and
`public_base_url()` (env `PUBLIC_BASE_URL`) is what invite links
(`GET /api/groups` -> `invite_base`) and the device pair payload are built from,
instead of the browser origin and `request.base_url`.
...
(metadata) and `GET /api/plugin/download`, both unauthenticated.
