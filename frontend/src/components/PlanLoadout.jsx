...
import { firstAvailableSavedSet, savedSetInUse, savedSetSaveDestinations,
...
function allSets(shortlist, catalog) {
  ...
  ;[...(shortlist.sets || []), ...(catalog || [])].forEach((set) => {
...
function plannedSetCounts(character, shortlist, active, catalog) {
  ...
  PLAN_SLOTS.forEach((def) => {
    ...
    if (!item || !itemSockets(item).some((socket) => socket.color === 'turquoise')) return
    ...
    if (!set || !setFitsHost(set, item, def.catalog)) return
...
function wornSets(character, shortlist, active, catalog) {
  ...
  const add = (name, bonuses, piece) => {
    const row = out.get(name) || { name, count: 0, bonuses: [], pieces: [] }
    row.count += 1
    if (piece && !row.pieces.includes(piece)) row.pieces.push(piece)
  ...
  ;(character?.gear || []).forEach((item) => (item.adornments || []).forEach((adorn) => {
    if (adorn.color !== 'turquoise' || !adorn.name) return
    const name = adorn.set_name || adorn.name
    const row = current.get(name) || { name, count: 0, bonuses: [], pieces: [] }
    row.count += 1
    const label = PLAN_SLOTS.find((slot) => slot.key === item.key)?.label || item.key
    const piece = item.name ? `${label}: ${item.name}` : label
    if (piece && !row.pieces.includes(piece)) row.pieces.push(piece)
  ...
  PLAN_SLOTS.forEach((def) => {
    ...
    if (hasChoice) {
      if (set && itemSockets(item).some((socket) => socket.color === 'turquoise')
          && setFitsHost(set, item, def.catalog)) {
        add(set.name, set.bonuses, item.name ? `${def.label}: ${item.name}` : def.label)
      }
      return
    }
    ;(item.adornments || []).forEach((adorn) => {
      ...
      add(adorn.set_name || adorn.name, adorn.stats?.adornment?.set_bonuses,
        item.name ? `${def.label}: ${item.name}` : def.label)
    })
  })
...
function WornSetCard({ set, full = false }) {
  return (
    <div className={`wornset${full ? ' full' : ' preview'}`} tabIndex={full ? undefined : 0}>
      ...
      <div className="wornsetpieces">
        {set.pieces.length ? set.pieces.join(' · ') : 'No pieces in the planned loadout'}
      </div>
      {full && !set.bonuses.length && (
        <p className="muted">No tier text recorded for this set.</p>
      )}
      {full && set.bonuses.map((bonus, i) => (
...
function projection(character, shortlist, active, catalog) {
  ...
  Object.entries(shortlist.adorn_slots || {}).forEach(([slot, choices]) => {
    ...
    Object.entries(choices || {}).forEach(([socket, choice]) => {
      ...
      if (choice) addStats(totals, choice.projection_stats || choice.stats, 1)
    })
  })
  ...
  PLAN_SLOTS.forEach((def) => {
    ...
    itemSockets(item).forEach((socket, index) => {
      ...
      if (overridden) {
        if (choices[index]) addStats(breakdown.adornments,
          choices[index].projection_stats || choices[index].stats, 1)
...
function WhiteAdornmentSocket({ adorn, selection, choices, changed, onChange }) {
  ...
  const carried = selection && !choices.some((choice) => choice.key === selection.key)
    ? [{
      value: selection.key, label: adornmentSummary(selection),
      menuLabel: <strong className="adornstat">{adornmentSummary(selection)}</strong>,
      hint: compactWhiteName(selection), group: 'Carried from slot',
      title: selection.name,
      icon: <SocketTile adorn={selection} color="white" />,
    }]
    : []
  const options = [{
  ...
  return (
    <span className="adornchoice" title={adornmentSummary(shown)}
      ...
      <Picker className={`adornpicker white${changed ? ' changed' : ''}`} value={value}
              onChange={(next) => onChange(next === '__equipped__' ? undefined
                : next === '__empty__' ? null
                  : next === selection?.key ? selection
...
function SetAdornmentSocket({ adorn, selection, sets, changed, onChange }) {
  ...
  return (
    <span onClick={(e) => e.stopPropagation()}>
      <Picker className={`adornpicker${changed ? ' changed' : ''}`} value={value}
              onChange={(next) => onChange(next === '__equipped__' ? undefined
...
function EquipmentSlot({ def, character, shortlist, active, focused, occupied,
  ...
  return (
    <div className={`planslot${focused ? ' focused' : ''}${occupied ? ' occupied' : ''}${!isCurrent && shown ? ' planned' : ''}`}
      ...
      {shown && (
        <AdornmentIcons item={shown} baselineItem={current} current={isCurrent}
...
function compatibleSets(sets, item, slot) {
  return (sets || []).map((set) => ({ ...set, piece: setPieceForSlot(set, slot) }))
    .filter((set) => setFitsHost(set, item, slot))
...
function AdornmentIcons({ item, baselineItem, current, sets, whiteAdornments, slot, installed,
                          whiteInstalled, onChange, onWhiteChange, compact = false }) {
  const adornments = itemSockets(item).map((socket) => socket.adorn)
  const baseline = itemSockets(baselineItem).map((socket) => socket.adorn)
  const baselineSet = baseline.find((adorn) => adorn.color === 'turquoise')
  const baselineSetName = adornmentName(baselineSet)
  const selectedSetName = installed === undefined
    ? adornments.find((adorn) => adorn.color === 'turquoise')?.set_name
      || adornments.find((adorn) => adorn.color === 'turquoise')?.name || null
    : installed
  const baselineWhites = baseline.filter((adorn) => adorn.color === 'white')
  let whiteIndex = 0
      ...
      {adornments.map((adorn, i) => (
        adorn.color === 'turquoise' ? (
          <SetAdornmentSocket key={`${adorn.color}-${i}`} adorn={adorn}
            selection={installed} sets={legal}
            changed={adornmentChanged(selectedSetName, baselineSetName)} onChange={onChange} />
        ) : adorn.color === 'white' ? (
          (() => {
            const original = baselineWhites[whiteIndex++] || null
            const hasSelection = Object.prototype.hasOwnProperty.call(
              whiteInstalled || {}, i)
            const selected = hasSelection ? whiteInstalled[i] : adorn
            return <WhiteAdornmentSocket key={`${adorn.color}-${i}`} adorn={adorn}
              selection={whiteInstalled?.[i]}
              changed={adornmentChanged(selected, original)}
    ...
    return (
      <Hover className="statbreakdown" width={280} card={card} block key={key}>
        <div className="planstatrow" tabIndex="0">
          <span>{label}</span>
          <span className={`planstatvalue ${state}`}>{precise(value)}{pct ? '%' : ''}</span>
          <em className={state}>
...
export default function PlanLoadout({ characters, recentCharacters, character,
                                     ...
                                     savedSetStatus, savedSetWorkingStatus,
                                     savedSetDirty, savedSetModified, completedTargets,
  ...
  const charOptions = (characters || []).map((c) => ({
    ...
    .filter((row) => !accountNames.has(
      String(row.lookup_name || row.lookupName || row.name).toLowerCase())
  ...
  const guestNeedsCharacter = !signedIn && !character?.character && !requestedCharacter
  const requestedPending = !character?.character && !!requestedCharacter
    ...
    <div className="card planloadout">
      <div className="loadouthead">
        ...
        <div className="loadoutwho">
          <h2>{character?.character?.display_name || character?.character?.name
            || (requestedCharacter
              ? `${lookupBusy ? 'Loading' : 'No record for'} ${requestedCharacter}`
              : 'No character loaded')}</h2>
        </div>
        ...
        <div className="loadoutcontrolrow">
          <span className="loadoutmeta">{character?.character
            ...
            : requestedCharacter
              ? lookupBusy ? `Looking up ${requestedCharacter} on Wuoshi…`
                : `No character record for ${requestedCharacter}.`
          ...
          <SavedSetControls sets={savedSets} slot={activeSavedSetSlot}
            ...
            workingStatus={savedSetWorkingStatus}
            ...
            completed={completedTargets} onImportGuest={onImportGuestSet}
      ...
      <div className="loadoutbody">
        <div className={`loadoutgear${guestNeedsCharacter || requestedPending
          ? ' awaitingcharacter' : ''}`}>
          ...
          {requestedPending && (
            <div className="guestgearlookup requeststate" role="status">
              <b>{lookupBusy ? `Looking up ${requestedCharacter}…`
                : `No character record for “${requestedCharacter}”`}</b>
            </div>
          )}
        </div>
...
function SavedSetControls({ sets, slot, ownerLoaded, signedIn, busy, status, dirty,
                            workingStatus, modified, resetDisabled, onReset, shortlist,
                            onRemoveItem, onToggleTrackedSet, onClearContents,
                            completed, onImportGuest,
  ...
  const saveDestinations = savedSetSaveDestinations(sets, slot)
  const updateDestination = saveDestinations.find((destination) => (
    destination.kind === 'update'))
  const replaceDestinations = saveDestinations.filter((destination) => (
  ...
  const guestConflicts = (sets || []).filter((row) => row._guest_conflict)
  const hasContents = contentCount || completedRows.length || guestConflicts.length
  ...
  useEffect(() => {
    ...
    const away = (event) => {
      if (!root.current?.contains(event.target)) {
        setPanel(null); setPending(null); setSaveTarget(null)
      }
    }
    const escape = (event) => {
      if (event.key === 'Escape') {
  ...
  const openStandalonePanel = (next) => {
    setPending(null)
    setSaveTarget(null)
    setPanel(next)
  }
  ...
  const saveTo = async (destination, name = destination?.name) => {
    if (!destination) return
    await onSave(destination.slot, name)
    if (pending) finishPending()
    else close()
  }

  const create = () => saveTo(available, draft)
  ...
  const replacementPanel = () => (
    <div className="plansavedpopover" role="alertdialog" aria-label="Uncommitted gear changes">
      ...
      <div className="plansavedpopoveractions">
        {modified && saveDestinations.length > 0 && (
          <button type="button" className="chip on" disabled={busy}
                  onClick={openSave}>Save and continue…</button>
        )}
    ...
    <div className="plansavedsets" ref={root}>
      ...
      <span className="plansavedanchor plansavedpickeranchor">
        <Picker className="plansavedpicker" value={slot ? String(slot) : ''}
                ...
                placeholder={used.length ? 'Load set…' : 'No saved sets'}
      ...
      <span className="plansavedanchor plansavedsaveanchor">
        <button type="button" className="chip on plansavedsave"
                disabled={busy || !ownerLoaded || !modified}
                aria-haspopup="dialog" aria-expanded={panel === 'save'}
                title="Save this plan to a new or existing gear set"
                onClick={() => (panel === 'save' ? close() : openSave())}>
          Save
        </button>
        {panel === 'save' && (
          <div className="plansavedpopover savechoices" role="dialog"
               aria-label="Save gear set">
            <b>{selected ? 'Save plan to' : 'Save draft to'}</b>
            <div className="plansaveddestinations">
              {updateDestination && dirty && (
                <button type="button" className="current" disabled={busy}
                        onClick={() => saveTo(updateDestination.row)}>
                  <span><small>Current set</small><b>{updateDestination.row.name}</b></span>
                  <em>Update</em>
                </button>
              )}
              {replaceDestinations.map(({ row }) => (
                <button type="button" disabled={busy} key={row.slot}
                        onClick={() => requestOverwrite(row)}>
                  <span><small>Saved set {row.slot}</small><b>{row.name}</b></span>
                  <em>Replace</em>
                </button>
              ))}
              {available && (
                <button type="button" className="new" disabled={busy}
                        onClick={openNew}>
                  <span><small>Empty slot {available.slot}</small><b>Create new set</b></span>
                  <em>New</em>
                </button>
              )}
            </div>
        ...
        {panel === 'new' && available && (
          <div className="plansavedpopover" role="dialog" aria-label="Create gear set">
            <b>Create new set</b>
            <p>This uses empty slot {available.slot}.</p>
            ...
            <div className="plansavedpopoveractions">
              <button type="button" className="chip on" disabled={busy}
                      onClick={create}>Create set</button>
              <button type="button" className="btnlink"
                      onClick={() => setPanel('save')}>Back</button>
            </div>
          </div>
        )}
        {panel === 'overwrite' && saveTarget && (
          <div className="plansavedpopover" role="alertdialog"
               aria-label={`Replace ${saveTarget.name}`}>
            <b>Replace {saveTarget.name}?</b>
            <p>Its saved targets will be replaced with this working plan.</p>
            <div className="plansavedpopoveractions">
              <button type="button" className="chip danger" disabled={busy}
                      onClick={() => saveTo(saveTarget)}>
                Replace set
              </button>
              <button type="button" className="btnlink"
                      onClick={() => setPanel('save')}>Back</button>
          ...
          <div className="plansavedpopover contents" role="dialog" aria-label="Set contents">
            ...
            <div className="plansavedcontentlist">
              ...
              {!!completedRows.length && (
                <details className="plansavedcompleted">
                  <summary>Completed {completedRows.length}</summary>
                  {completedRows.map((row) => (
                    <label key={row.key}>
                      <input type="checkbox" checked onChange={() => (
                        row.kind === 'item'
                          ? onRemoveItem(row.target.equip_slot, row.target.page_title)
                          : onToggleTrackedSet(row.target))} />
                      <span>
                        <small>{row.planned_slot || (row.kind === 'item' ? 'Gear' : 'Set adornment')}</small>
                        <b>{row.name}</b>
                        <em>Equipped {row.first_seen_ts
                          ? new Date(row.first_seen_ts * 1000).toLocaleDateString() : 'previously'}</em>
                      </span>
                    </label>
                  ))}
                </details>
              )}
              {!!guestConflicts.length && <span className="seclabel">Guest copies</span>}
              {guestConflicts.map((row) => (
                <div className="plansavedguestcopy" key={`guest:${row.slot}`}>
                  <span><small>Slot {row.slot}</small>
                    <b>{row._guest_conflict.name}</b></span>
                  <button type="button" className="btnlink" disabled={busy}
                          onClick={() => onImportGuest(row.slot)}>
                    Import, replacing {row.name}
                  </button>
                </div>
              ))}
            </div>
      ...
      <span className="plansavedanchor plansavedrenameanchor">
        <button type="button" className="btnlink plansavedaction"
                ...
                onClick={() => {
                  if (!selected) return
                  setDraft(selected.name); openStandalonePanel('rename')
                }}>Rename</button>
      ...
      <span className="plansavedanchor plansaveddeleteanchor">
        <button type="button" className="btnlink plansavedaction danger"
                disabled={busy || !selected}
                onClick={() => openStandalonePanel('delete')}>Delete</button>
      ...
      {(!selected || dirty) && (
        <span className={`plansaveddirty${dirty ? ' on' : ''}`}
              title={workingStatus || undefined}>
          {workingStatus || 'Equipped'}
        </span>
      )}
