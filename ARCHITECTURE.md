...
| `docs/planner.md` | The Gear Planner (`/plan`, in the nav, signed-out) — reader-chosen expansions, Census/Lexicon current gear, five named local/account-backed target plans per canonical public character, additive obtained-item reconciliation, remembered searches, slot-aware candidates and adornments, projected stats, gear search, explicit Fabled→Mythical class-epic suggestions, and a contextual zone→mob/quest prerequisite list. **Phases 0-2 COMPLETE** (`backend/planner/`, schema v52) |
